...
def _eval_once(feed, saver, summary_writer, top_1_op, top_5_op, summary_op):
    """Runs Eval once.
            ...
            while step < num_iter and not coord.should_stop():
                ...
                if step % 20 == 0:
                    ...
                    print('%s: [%d batches out of %d] (%.1f examples/sec; %.3f'
                          'sec/batch)' % (datetime.now(), step, num_iter, examples_per_sec, sec_per_batch))
                    start_time = time.time()
...
def evaluate(dataset):
    ...
    with tf.Graph().as_default():
        ...
        while True:
            _eval_once(feed, saver, summary_writer, top_1_op, top_5_op, summary_op)
            if FLAGS.run_once:
                break
