...
from tensorflow.contrib.slim.python.slim import scopes
from tensorflow.contrib.slim.python.slim import variables
...
class VariablesTest(tf.test.TestCase):
  ...
  def testVariableWithReplicaDeviceSetter(self):
    with self.test_session():
      ...
      #self.assertDeviceEqual(a.initial_value.device, '/job:ps/task:0/cpu:0')
