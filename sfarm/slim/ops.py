...
def batch_norm(inputs,
  ...
  with vs.variable_op_scope([inputs], scope, 'BatchNorm', reuse=reuse):
    ...
    moving_collections = [moving_vars, ops.GraphKeys.MOVING_AVERAGE_VARIABLES]
...
def conv2d(inputs,
  ...
  """
  with vs.variable_op_scope([inputs], scope, 'Conv', reuse=reuse):
    ...
    if batch_norm_params is not None:
      ...
      bias_initializer = init_ops.constant_initializer(bias)
      biases = variables.variable('biases',
                                  shape=bias_shape,
                                  initializer=bias_initializer,
                                  trainable=trainable,
                                  restore=restore)
      outputs = nn.bias_add(conv, biases)
    if activation:
      outputs = activation(outputs)
    return outputs
...
def fc(inputs,
  ...
  """
  with vs.variable_op_scope([inputs], scope, 'FC', reuse=reuse):
    ...
    if batch_norm_params is not None:
      ...
      biases = variables.variable('biases',
                                  ...
                                  trainable=trainable,
                                  restore=restore)
      outputs = nn.xw_plus_b(inputs, weights, biases)
    if activation:
      outputs = activation(outputs)
    return outputs
...
def one_hot_encoding(labels, num_classes, scope=None):
  ...
  """
  with ops.op_scope([labels], scope, 'OneHotEncoding'):
    batch_size = labels.get_shape()[0]
    indices = array_ops.expand_dims(math_ops.range(0, batch_size), 1)
    labels = math_ops.cast(array_ops.expand_dims(labels, 1), indices.dtype)
    concated = array_ops.concat(1, [indices, labels])
    onehot_labels = sparse_ops.sparse_to_dense(
        concated, array_ops.pack([batch_size, num_classes]), 1.0, 0.0)
...
def max_pool(inputs, kernel_size, stride=2, padding='VALID', scope=None):
  ...
  """
  with ops.op_scope([inputs], scope, 'MaxPool'):
    kernel_h, kernel_w = _two_element_tuple(kernel_size)
    stride_h, stride_w = _two_element_tuple(stride)
    return nn.max_pool(inputs,
...
def avg_pool(inputs, kernel_size, stride=2, padding='VALID', scope=None):
  ...
  Returns:
    a tensor representing the results of the pooling operation.
  """
  with ops.op_scope([inputs], scope, 'AvgPool'):
    kernel_h, kernel_w = _two_element_tuple(kernel_size)
    stride_h, stride_w = _two_element_tuple(stride)
    return nn.avg_pool(inputs,
                          ksize=[1, kernel_h, kernel_w, 1],
                          strides=[1, stride_h, stride_w, 1],
                          padding=padding)
...
def dropout(inputs, keep_prob=0.5, is_training=True, scope=None):
  ...
  if is_training and keep_prob > 0:
    with ops.op_scope([inputs], scope, 'Dropout'):
      return nn.dropout(inputs, keep_prob)
  else:
    return inputs
...
def flatten(inputs, scope=None):
  ...
  k = dims.num_elements()
  with ops.op_scope([inputs], scope, 'Flatten'):
    return array_ops.reshape(inputs, [-1, k])
...
def repeat_op(repetitions, inputs, op, *args, **kwargs):
  ...
  with vs.variable_op_scope([inputs], scope, 'RepeatOp'):
    tower = inputs
